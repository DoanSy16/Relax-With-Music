...
<section>
	<c:forEach var="video" items="${videoTrend}">
...
<section>
	<div class="dark-grey-text text-center p-5 rgba-white-strong"
		...
		<h3 class="font-weight-bold mb-4 pb-2">Xu hướng</h3>
		<div class="row">
			<c:forEach var="video" items="${videoTrend}">
									...
									<div>
										<c:choose>
											...
											<c:otherwise>
												...
												<a href="./main?action=video&idVideo=${video.id}"
													class="btn btn-green btn-rounded font-weight-bold w-72px"><i class="fa fa-play fs-1 white-text "></i></a>

											</c:otherwise>
										</c:choose>
									</div>
									<div
										class="${(sessionScope.user.position=='Admin')?'show':'hidden'}">
